...
class RangeCmtChangedEvent(Event):
    ...
    def __call__(self):
        if self.kind == ida_range.RANGE_KIND_FUNC:
            ...
            ida_funcs.set_func_cmt(func, self.cmt, self.rptble)
        elif self.kind == ida_range.RANGE_KIND_SEGMENT:
            segment = ida_segment.getseg(self.start_ea)
            ida_segment.set_segment_cmt(segment, self.cmt, self.rptble)
        else:
...
class LocalTypesChangedEvent(Event):
    ...
    def __init__(self, local_types):
        ...
        if local_types is not None:
            self.local_types.extend(local_types)
...
class StrucCmtChangedEvent(Event):
    ...
    def __call__(self):
        ...
        if self.smname:
            ...
            ida_struct.set_member_cmt(mptr, self.cmt, self.repeatable_cmt)
        else:
            ida_struct.set_struc_cmt(sptr.id, self.cmt, self.repeatable_cmt)
